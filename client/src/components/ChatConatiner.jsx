...
const ChatConatiner = () => {
  ...
  const handleSendImage = async (e) => {
    ...
    render.onloadend = async () => {
      await sendMessages({image: render.result})
      e.target.value = ''
    }
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6 '>
        {messages.map((msg, index) => {
          if (!msg) return null; // Skip undefined/null messages
          return (
            <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
              ...
              <div className='text-center text-xs'>
                <img src={msg.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.profile_ayush} alt="" className='w-7 rounded-full' />
